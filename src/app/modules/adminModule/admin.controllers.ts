...
const createAdmin = async (req: Request, res: Response) => {
  ...
  const adminId = IdGenerator.generateId();
...
const changeAdminProfileImage = async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  const adminImagePath = await fileUploader(files as FileArray, `admin-image-${admin.adminId}`, 'image');
  const updateAdmin = await adminServices.updateSpecificAdmin(id, {
    image: adminImagePath as string,
  });
